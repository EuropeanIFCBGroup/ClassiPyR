...
server <- function(input, output, session) {
  ...
  load_worms_map <- function(db_folder) {
    ...
    # Fallback: legacy JSON map; migrate into DB if present.
  ...
  # WoRMS query/matching helpers live in R/worms.R
  ...
  worms_query_input_id <- function(i) {
    paste0("worms_query_", i)
  }
  ...
  observeEvent(input$apply_worms_matches, {
    ...
    if (rename_requested) {
      for (i in seq_len(nrow(matched))) {
        ...
        if (is.na(new_name) || !nzchar(new_name) || identical(old_name, new_name)) {
          next
        }
      ...
      # Keep temporary class list file in sync when in scratch mode
  ...
  apply_settings <- function(close_modal = FALSE, notification = "Settings saved.") {
    ...
    # If DB folder changed, reload persisted AphiaID map from the selected DB.
